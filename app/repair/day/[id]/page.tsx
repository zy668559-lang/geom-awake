    ...
    <div className="min-h-screen bg-[#F8F9FA] flex flex-col">
      <nav className="p-6 flex items-center justify-between bg-white shadow-sm">
        ...
        <span className="font-black text-slate-800">
          {currentPack.label} · Day {content.day}
        </span>
        <div className="w-20" />
      </nav>
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full p-6 space-y-6 pb-24">
        <div className="bg-white rounded-[32px] p-8 shadow-sm">
          ...
          <p className="text-2xl font-black text-slate-800 leading-relaxed italic">{content.command}</p>
        </div>
        ...
        <div className="bg-white rounded-[32px] p-8 shadow-sm">
          ...
          <div className="space-y-3">
            {content.microPractice.map((problem, idx) => (
              <p key={`${content.day}-${idx}`} className="text-lg font-bold text-slate-700 leading-relaxed">
                {idx + 1}. {problem}
              </p>
            ))}
          </div>
        ...
        <div className="bg-[#667EEA]/5 border border-[#667EEA]/10 rounded-[32px] p-8">
          ...
          <p className="text-lg font-medium text-slate-600 italic">“{content.reflectionPrompt}”</p>
        </div>

        <div className="bg-white rounded-[32px] p-8 shadow-sm border border-slate-100">
          <h2 className="text-sm font-bold text-slate-400 tracking-widest uppercase mb-3">家长提示</h2>
          <p className="text-lg text-slate-700 leading-relaxed">{content.parentTip}</p>
        </div>
        ...
        <button
          onClick={() => {
            ...
            router.push(`/repair/submit?cause=${selectedCause}&day=${content.day}`);
          }}
        ...
        >
          {content.day === 7 ? "Day7完成，开始复检" : "我写好了，提交反馈"}
